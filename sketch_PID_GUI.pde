...
boolean pathTrack = false;
int dir = 0;
float trackSpeed = 0.001;
...
void setup(){
  ...
  cp5.addSlider("looptime")
     ...
     ;
  cp5.addTextlabel("TrackerSpeed")
                    .setText("Tracking Speed")
                    .setPosition(0,460)
                    .setColorValue(color(255,0,0))
  ...
  cp5.addSlider("trackSpeed")
     .setPosition(10,480)
     .setWidth(200)
     .setRange(0.00005,0.001) // values can range from big to small as well
     .setValue(0.0001)
...
public void PathTracking(int theValue) {
    pathTrack = !pathTrack;
    xr = xr + 0.01;
}
...
while(1==1) {
    ...
    if(haplyBoard.data_available()){
      ...
      if(!pathTrack){
        if(dir==1 && yr >= 0.5)
          dir = 2;
        else if(dir == 2 && xr <= -0.5)
          dir = 3;
        else if(dir == 3 && yr <= -0.5)
          dir = 0;
        else if(dir == 0 && xr >= 0.5)
          dir = 1;
